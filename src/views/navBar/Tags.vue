...
export default {
  ...
  methods: {
    handleClose(item) {
      //关闭当前标签页,并打开前一个标签页
      let index = this.list.findIndex((value) => value.path === item.path)
      this.list.splice(index, 1)
      this.$store.commit('setTagNavList', this.list)
    ...
    isCurrentTag(item) {
      return this.$route.path === item.path ? true : false
    },
    handleTagsOption(type) {
      if (type === 'close-all') {
        ...
        this.$store.commit('setTagNavList', [firstPage, currentRouth])
      }
    },
    handlescroll(e) {
      let type = e.type
